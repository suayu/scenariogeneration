...
BASE=ROOT/'experiments/riskweaver_multiagent_closed_20260917_r1'
MEMORY=ROOT/'experiments/riskweaver_multiagent_api_1789627364562698612/memory.json'
EXTRA_OVERRIDES=[] # 仅用于显式冻结的新诊断批次；四组对照默认无额外参数。
SMOKE_SCENE_INDEX=3
SMOKE_STEPS=80
SMOKE_MAX_PLANNING_CALLS=6
...
SOURCE += ['policies/multiagent_planner.py','policies/multiagent_memory.py']
ARMS=[(name,'trajectory_only',True) for name in
...
def freeze():
    ...
    if not manifest.exists():
        write(manifest,dict(seed=42,scene_files=items,model='qwen3.5-plus',provider='dashscope',
                            ...
                            smoke_scene_index=SMOKE_SCENE_INDEX,smoke_steps=SMOKE_STEPS,formal_steps=400,
                            smoke_max_planning_calls=SMOKE_MAX_PLANNING_CALLS,formal_max_planning_calls=10,
...
def run_arm(name,mode,profile,phase,scenario_manifest,source_hashes):
    ...
    overrides=['sim.seed=42','+sim.continue_after_off_route=true',
               f'sim.llm.multiagent.mode={name}',f'sim.llm.multiagent.memory_path={MEMORY}',
               f'sim.evaluation.max_scenarios={1 if phase=="smoke" else 20}',
               f'sim.steps={SMOKE_STEPS if phase=="smoke" else 400}',
               ...
               'sim.llm.model_names=[qwen3.5-plus]',f'sim.llm.max_planning_calls={SMOKE_MAX_PLANNING_CALLS if phase=="smoke" else 10}',
    ...
    command=['bash','scripts/run_with_llm_provider.sh','dashscope',sys.executable,'run_simulation.py',*overrides,*EXTRA_OVERRIDES]
    ...
    try:
        ...
        # 诊断阶段保留所有组的失败；任何服务失败立即停止后续组以保护免费额度。
        rows=[json.loads(p.read_text()) for p in (directory/'movies').glob('scenario_*/attempt_result.json')]
        if not rows or any(r.get('planner_service_failure_count') for r in rows):
            raise
        summary=dict(smoke_passed=False,error=str(error),results=rows)
