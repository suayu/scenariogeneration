"""原始多背景场景：只在攻击窗口增强非目标车道路软引导。"""
import os
import sys
import time
import launch_multiagent_matrix as matrix

matrix.BASE=matrix.ROOT/'experiments'/('riskweaver_attack_route_smoke_'+str(time.time_ns()))
matrix.SMOKE_SCENE_INDEX=3
matrix.SMOKE_STEPS=80
matrix.SMOKE_MAX_PLANNING_CALLS=1
matrix.ARMS=[('single','trajectory_only',True)]
matrix.SOURCE += ['policies/trajectory_projection.py','tests/probe_attack_only_route.py']
