...
class Guidance(ABC):
    def __init__(self, loss_timesteps=None, filter_timesteps=None, loss_scale=1.0) -> None:
        ...
        self.device = None
        # 数值断言会同步 CUDA；默认关闭，仅供定位 NaN 时手动开启。
        self.validate_gradients = False
    ...
    def calculate_grad(self, action, state, a_t, grad_wrt, data_batch_for_guidance):
        """
            ...
            参数:
                action: Tensor,表示清理后的轨迹或噪声轨迹。
                state: Tensor,resulting traj
                a_t: Tensor,表示带噪声的轨迹输入tau_k。
                grad_wrt: str,指定计算梯度的目标,可选值为 "clean_guide" 或 "noisy_guide"。
                data_batch_for_guidance: dict,包含与指导相关的数据。
...
class CombinedLossCalculator(Guidance):
    ...
    def calculate_loss(self, action, state, data_batch_for_guidance):
        ...
        参数:
            action: Tensor,表示轨迹动作,形状为 (B*N, T, ...)。
            state: Tensor,表示轨迹状态,形状为 (B*N, T, ...)。
            data_batch_for_guidance: dict,包含与指导相关的数据。

        返回:
            total_loss: Tensor,总损失,形状为 (B*N, T)。
        """
        # 初始化总损失
...
class RouteLossCalculator(Guidance):
    ...
    def calculate_loss(self, action, state, params):
        ...
        # LLM 指定的攻击车可暂时离开道路以执行对抗机动；其他车辆仍保留道路约束。
        route_exempt_mask = params.get("guidance_route_exempt_mask")
        if route_exempt_mask is not None:
            batch_size = int(params["batch_size"])
            num_samples = int(params["num_samples"])
            route_exempt_mask = enlarge_batch_samples(
                route_exempt_mask.to(
                    device=cost.device,
                    dtype=cost.dtype,
                ),
                batch_size,
            ...
            cost = cost * (1.0 - route_exempt_mask[:, None].clamp(0.0, 1.0))
