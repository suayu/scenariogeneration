...
def get_checkpoint(
    ...
    """
    根据本地目录获取检查点和配置文件路径。
    如果指定了 @ckpt_dir,函数将查找本地目录,并返回包含 @ckpt_key 的检查点文件及其对应的 config.json 文件。

    参数:
    ckpt_key (str):唯一标识检查点文件的字符串，例如 `iter50000.ckpt`  
    ckpt_dir (str):（可选）包含指定检查点的本地目录  
    ckpt_root_dir (str):（可选）函数将搜索检查点的根目录  
    download_tmp_dir (str):检查点的临时存储目录

    返回值:
    ckpt_path (str):检查点文件的路径  
    cfg_path (str):config.json 文件的路径
    """
...
# /home2/zhaoyx/safe-sim/safesim_checkpoints/nusc_dynUnicycle_gl0_yrl0_tfTrue_4130553/run0/config.json
# /home2/zhaoyx/safe-sim/safesim_checkpoints/nusc_dynUnicycle_gl0_yrl0_tfTrue_4130553/run0/checkpoints/iter94000_ep6_valLoss0.06.ckpt
