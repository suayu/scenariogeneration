...
def parse_scene_centric(batch: dict, rasterize_mode:str):
    """
    解析场景中心模式的批量数据，并根据栅格化模式处理地图信息。

    参数:
        batch (dict): 输入的批量数据，包含场景的观测信息。
        rasterize_mode (str): 栅格化模式，支持 "none"、"point" 和 "square"。

    返回:
        dict: 包含解析后的场景数据。
    """

    # 解析轨迹数据，获取未来和历史的位置信息、航向角、速度以及可用性掩码
...
def parse_trajdata_batch(batch: dict,rasterize_mode="point"):
    """
    解析 trajdata 的批量数据，支持场景中心和智能体中心两种模式。
    
    参数:
        batch (dict): 输入的批量数据，包含场景或智能体的观测信息。
        rasterize_mode (str): 栅格化模式，默认为 "point"。
            - "point": 使用点的方式进行栅格化。
            - "square": 使用方形区域进行栅格化。
            - "none": 不进行栅格化。
    
    返回:
        dict: 更新后的批量数据，包含解析后的信息。
    """
