...
class EnvUnifiedSimulation(BaseEnv, BatchedEnv):
    ...
    def step(self, actions: RolloutAction, num_steps_to_take: int = 1, render=False):
        ...
        参数：
        actions (RolloutAction)：用于控制自身和/或代理的动作
        num_steps_to_take (int)：要执行的环境步数。必须小于或等于输入动作的长度
        render (bool)：是否渲染状态和动作并返回渲染结果
        """
...
class EnvSplitUnifiedSimulation(EnvUnifiedSimulation):
    ...
    def get_observation(self,split_ego=None,return_raw=False, include_ego_obs=False):
        """
        split_ego: 是否将ego和其他agent的观测分开返回
        split_ego: 是否返回原始的观测数据
        include_ego_obs: 是否在返回的agent观测中包含ego的观测

        返回值:包含 'ego', 'agents'两个键的字典
        """
        ...
        if split_ego:
            ...
            ego_idx = self.ego_indices
            agent_idx = self.other_agent_indices
            # raw_obs 是未经 collation 的轨迹数据批次元素对象
            ego_obs_raw = [raw_obs[idx] for idx in ego_idx]
            # 调用 collate 函数，将批次元素转换为轨迹数据批次对象
        ...
        self.timers.toc("get_obs")
        return self._cached_observation # 返回一个字典，包含 ego 和 agents 的观测信息
    ...
    def _step(self, step_actions: RolloutAction, num_steps_to_take):
        """
        内部步进方法：将策略输出的动作应用到环境中，更新所有智能体的状态。

        该方法负责将动作从智能体局部坐标系转换到世界坐标系，并驱动每个场景中的每个智能体向前推进。
        支持多步执行(num_steps_to_take),但动作序列长度需与步数匹配。
        同时负责日志记录（若启用）和缓存清理。

        Args:
            step_actions: 动作对象，可能包含 ego 和 agents 的独立动作（若分离）。
            num_steps_to_take: 要连续执行的动作步数,通常等于n_step_action。
        """
        if self.is_done():
            raise SimulationException("Cannot step in a finished episode")
