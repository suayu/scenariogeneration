...
class DiffusionTrafficModel(pl.LightningModule):
    """
    扩散交通模型类。封装了扩散模型的训练、验证和推理逻辑，核心网络为 RasterizedDiffusionModel。
    主要用于生成多智能体的未来轨迹,支持引导和条件采样。
    在 TBSIM 中作为可学习的策略被 PolicyComposer 加载。供 build_single_policy 或 build_dual_policies 调用。

    继承关系:pl.LightningModule → DiffusionTrafficModel
    """
    def __init__(self, algo_config, modality_shapes, do_log=True):
        ...
        # 创建核心扩散网络 RasterizedDiffusionModel
        # 该网络包含：
        # - 图像编码器（栅格地图 CNN）
        # - 历史轨迹编码器（MLP 或 RNN）
        # - 扩散去噪网络（通常为 UNet 或 Transformer）
        # - 引导模块（用于条件生成）
        self.nets["policy"] = RasterizedDiffusionModel(
            model_arch=algo_config.model_architecture,
            input_image_shape=modality_shapes["image"],          # [C, H, W] 栅格图尺寸
            map_feature_dim=algo_config.map_feature_dim,         # 地图特征维度

            dynamics_config = {
                            'feature_dim':algo_config.map_feature_dim,
                            'state_dim': 3,                                  # 状态维度 (x, y, yaw)
                            'num_steps': algo_config.horizon,                # 预测步数
                            'dynamics_type': algo_config.dynamics.type,
                            'dynamics_kwargs': algo_config.dynamics,
                            'step_time': algo_config.step_time
                        },
            ...
            diffnet_config={                                      # 扩散网络配置
                "horizon": algo_config.horizon,
                "transition_dim": algo_config.transition_dim,     # 状态转移维度
                "context_dim": algo_config.context_dim,           # 上下文条件维度
            ...
            do_guidance = algo_config.get("do_guidance", False),    # 是否启用引导
            guide_config = algo_config.guide_config                 # 引导参数（损失权重、学习率等）
        )
    ...
    def forward(self, obs_dict):
        """
        前向推理：直接调用策略网络，返回预测的轨迹。

        Args:
            obs_dict: 输入观测字典（包含历史、地图等）。

        Returns:
            predictions: 网络输出的预测结果，通常包含 positions, yaws 等。
        """
        return self.nets["policy"](obs_dict)["predictions"]

    def _compute_metrics(self, pred_batch, sample_batch, data_batch):
        """
        计算评估指标:ADE(平均位移误差)、FDE(最终位移误差)和多样性(ATD/FTD)。

        Args:
            pred_batch: 后验预测结果（单模态或确定性输出）。
            sample_batch: 采样得到的多模态轨迹样本（从扩散采样）。
            data_batch: 真值数据（包含目标轨迹）。
    ...
    def get_plan(self, obs_dict, **kwargs):
        ...
        Returns:
            plan: Plan 对象,包含位置、航向和有效性标志。
            info: 空字典。
        """
        preds = self(obs_dict)   # 调用 forward
        plan = Plan(
            positions=preds["positions"],
            yaws=preds["yaws"],
            availabilities=torch.ones(preds["positions"].shape[:-1]).to(
    ...
    def get_action(self, obs_dict, sample=True, plan_samples=None, **kwargs):
        """
        核心推理接口:返回动作(Action)用于仿真闭环控制。

        Args:
            obs_dict: 观测字典。
            sample: 若为 True,则进行扩散采样生成多模态轨迹;否则使用后验预测。 默认为True。
            plan_samples: 可选的规划目标(如目标位置),用于条件生成。

        Returns:
            action: Action 对象，包含轨迹位置和航向。
            info: 附加信息字典，包含所有采样样本、车道变换状态、去噪过程等。

        obs_dict keys: 'data_idx', 'scene_ts', 'dt', 'agent_type', 'curr_agent_state', 'agent_hist', 'agent_hist_extent', 'agent_hist_len', 
        'agent_fut', 'agent_fut_extent', 'agent_fut_len', 'num_neigh', 'neigh_indices', 'neigh_types', 'neigh_hist', 'neigh_hist_extents', 
        'neigh_hist_len', 'neigh_fut', 'neigh_fut_extents', 'neigh_fut_len', 'robot_fut_len', 'map_names', 'maps', 'maps_resolution', 'vector_maps', 
        'rasters_from_world_tf', 'agents_from_world_tf', 'scene_ids', 'history_pad_dir', 'extras', 'image', 'drivable_map', 'target_positions', 
        'target_yaws', 'target_speeds', 'target_availabilities', 'history_positions', 'history_yaws', 'history_availabilities', 'curr_speed', 'centroid', 
        'yaw', 'type', 'extent', 'raster_from_agent', 'agent_from_raster', 'raster_from_world', 'agent_from_world', 'world_from_agent', 
        'all_other_agents_history_positions', 'all_other_agents_history_yaws', 'all_other_agents_history_availability', 
        'all_other_agents_history_availabilities', 'all_other_agents_curr_speed', 'all_other_agents_future_positions', 'all_other_agents_future_yaws', 
        'all_other_agents_future_availability', 'all_other_agents_types', 'all_other_agents_extents', 'all_other_agents_history_extents', 'scene_index', 
        'track_id', 'ego_idx', 'ego_plan'    
        """

        obs_dict = dict(obs_dict)

        # 若提供了 plan_samples 且模型支持目标条件，则将目标信息注入观测
        ...
        if sample:
            # 扩散采样：生成多个轨迹样本（N 个）
            preds_dict = self.nets["policy"].sample(obs_dict) # 调用 RasterizedDiffusionModel.sample 方法
            preds = preds_dict["predictions"]  # [B, N, T, 3] 其中 3 表示 (x, y, yaw)
            # 取第一个样本作为执行的动作（通常为最优模式或主模式）
            ...
            info = dict(
                ...
                denoising_predictions = preds_dict.get("denoising_predictions", None),  # 去噪过程中间结果
                # adv_proposals = preds_dict.get("adv_proposals", None),
            )
            # print("action:",action_preds["positions"], " info:",info)
        else:
