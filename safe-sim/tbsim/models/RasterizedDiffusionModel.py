...
class RasterizedDiffusionModel(nn.Module):
    ...
    def __init__(
        ...
        if self.do_guidance:
            self.Loss_Calculater = initialize_loss_calculator(self.guide_config) # 生成引导损失计算器，用于计算引导损失和梯度
    ...
    def forward(self, data_batch, guide_sample_fn=None, data_batch_for_guidance=None)-> Dict[str, torch.Tensor]:
        ...
        Args:
            data_batch: 输入批次，需包含 image, agent_hist, neigh_hist, 以及可选 adv_proposals_dict。
            guide_sample_fn: 引导函数（可选）。
            data_batch_for_guidance: 引导所需数据。

        Returns:
            out_dict: 包含 predictions(positions, yaws)及可能的 denoising_predictions。
        """
        # 控制哪些智能体需要预测
        ...
        # 调用扩散采样器
        actions = self.diffusion.sample( # 调用 DiffusionTraj.sample 方法，执行扩散采样
            cond_feat,
            forward_mode="sample",
    ...
    def sample(self, data_batch) -> Dict[str, torch.Tensor]:
        ...
        参数：
        data_batch (Dict)：包含场景和智能体信息的输入批次

        返回值：
        Dict[str, torch.Tensor]：包含预测轨迹和状态的字典
        """
    ...
    def _prepare_guidance_data(self, data_batch) -> Dict:
        ...
        # Scenario Dreamer 的自车不由扩散模型控制，仅作为联合风险损失的固定参考。
        # 这些可选字段不会改变 Safe-Sim 原始数据集与无引导采样路径。
        if "scenario_ego_state" in data_batch:
        ...
        if "guidance_route_exempt_mask" in data_batch:
            guidance_data["guidance_route_exempt_mask"] = data_batch[
                "guidance_route_exempt_mask"
            ]
        # Scenario Dreamer 使用以下可选字段调度攻击者与自车的分阶段接触损失。
        if "guidance_attack_active" in data_batch:
            guidance_data["guidance_attack_active"] = data_batch["guidance_attack_active"]
        if "guidance_attack_age_frames" in data_batch:
            guidance_data["guidance_attack_age_frames"] = data_batch[
                "guidance_attack_age_frames"
            ]
        if "scenario_curr_acceleration_world" in data_batch:
