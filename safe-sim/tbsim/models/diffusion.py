...
class DiffusionTraj(Module):
    """
    扩散模型采样引擎。
    负责管理噪声调度，执行去噪循环，支持引导和部分扩散替换。
    与网络结构解耦，通过 `net` 调用去噪模型。
    """
    def __init__(self, net, var_sched: VarianceSchedule, predict_epsilon=False):
        ...
        Args:
            net: 去噪网络,接受带噪输入、时间步和条件，输出预测的干净轨迹或噪声。
            var_sched: 方差调度器，提供 alpha_bar, beta, sigma 等序列。
            predict_epsilon: 若 True,网络预测噪声 ε；若 False,预测干净样本 x0。
        """
        super().__init__()
        self.net = net
        self.var_sched = var_sched
        self.predict_epsilon = predict_epsilon
        # 默认关闭仅用于开发期的同步断言，避免推理循环发生隐式 CUDA 同步。
        self.debug_checks = False
        # 仅包围 U-Net 去噪器的可选 FP16 加速；动力学和引导梯度始终保留 FP32。
        self.autocast_denoiser = False
    ...
    def sample(
        ...
        参数:
            context:交通情境条件
            num_points:轨迹中的点数
            num_samples:扩散模型的并行采样数量
            starting_noisy_a_t:起始噪声轨迹
            initial steps:起始噪声轨迹在截断扩散中的当前扩散步数
            adv_indices:要替换的噪声轨迹索引
        Returns:
            Tensor 或 dict: 若 guide_sample_fn 为 None,返回形状 [B, num_samples, T, point_dim];
                否则返回 {"sampled_trajectories": ..., "denoised_trajectories": ...}
        """
        batch_size = context.size(0)

        # 扩展 batch 维度，复制 num_samples 份。
        context = enlarge_batch_samples(context, batch_size, num_samples)
        if current_states is not None:
            current_states = enlarge_batch_samples(current_states, batch_size, num_samples)
            # 仅在显式调试时验证复制不变量；常规推理不应为断言同步 GPU。
        ...
        # 在目标设备直接创建采样噪声，避免 CPU 分配后再复制。
        if bestof:
            a_T = torch.randn([batch_size * num_samples, num_points, point_dim], device=context.device)
        else:
            a_T = torch.zeros([batch_size * num_samples, num_points, point_dim], device=context.device)
        ...
        for t in range(self.var_sched.num_steps, 0, -stride):
            ...
            with torch.enable_grad() if guide_sample_fn is not None else torch.no_grad():
                ...
                # 复用调度器中的同值 beta，避免 Python 索引列表且保持时间嵌入语义。
                beta = self.var_sched.betas[t].expand(batch_size * num_samples)
                ...
                if self.predict_epsilon:
                    ...
                    with torch.autocast(
                        device_type=tau_t.device.type,
                        dtype=torch.float16,
                        enabled=self.autocast_denoiser and tau_t.device.type == "cuda",
                    ):
                        a0_predict = self.net(
                            tau_t, context, time=beta, current_states=current_states, output_type="control"
                        )
                    # 后续动力学、损失及其梯度统一回到 FP32，避免约束边界出现溢出。
                    a0_predict = a0_predict.float()

                    # ---- 引导修正（若 grad_wrt=="clean_guide"） ----
        ...
        if guide_sample_fn is None:
            # 无引导时直接返回形状 [B, num_samples, T, point_dim]
        ...
        if guide_sample_fn is not None:
            ...
            # 引导函数提供的 filter 方法，根据损失选择最佳样本（如最小碰撞损失）
    ...
    def n_step_guided_p_sample(
        ...
        主要用于在去噪过程中注入约束（如避免碰撞、逼近锚点）。

        Args:
            a_t: 当前噪声（用于引导损失可能需要的上下文）。
            action: 待修正的预测 clean 或 noisy。
            guide: 引导对象，提供 calculate_grad 方法。
            beta: 当前时间步的噪声方差。
            current_states: 当前状态（用于动力学）。
            ... 其他超参数。
        ...
        for i in range(n_guide_steps):
            ...
            if self.net.dyn is not None:
                ...
                raise NotImplementedError
            # 计算引导损失对 action 的梯度,计算route、collision、ttc、causecollision
    ...
    def add_diffusion_noise(self, a_0, t=10, sample_size=20):
        """
        对给定的干净轨迹添加扩散噪声，用于部分扩散。

        Args:
            a_0: 干净轨迹，形状 [num_adv, num_proposals, T, 2]。
            t: 扩散步数。
            sample_size: 目标样本数（必须能被 num_proposals 整除）。

        Returns:
            加噪后的轨迹，形状 [num_adv, sample_size, T, 2]。
        """
