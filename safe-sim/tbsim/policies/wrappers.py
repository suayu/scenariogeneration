...
class Pos2YawWrapper(Wrappers):
    ...
    def __init__(self, policy, dt, yaw_correction_speed):
        ...
        self.policy = policy # DiffusionTrafficModel
        self._dt = dt
    ...
    def get_action(self, obs, **kwargs):
        """
        obs.keys: 'data_idx', 'scene_ts', 'dt', 'agent_type', 'curr_agent_state', 'agent_hist', 'agent_hist_extent', 'agent_hist_len', 'agent_fut', 
        'agent_fut_extent', 'agent_fut_len', 'num_neigh', 'neigh_indices', 'neigh_types', 'neigh_hist', 'neigh_hist_extents', 'neigh_hist_len', 'neigh_fut', 
        'neigh_fut_extents', 'neigh_fut_len', 'robot_fut_len', 'map_names', 'maps', 'maps_resolution', 'vector_maps', 'rasters_from_world_tf', 
        'agents_from_world_tf', 'scene_ids', 'history_pad_dir', 'extras', 'image', 'drivable_map', 'target_positions', 'target_yaws', 'target_speeds', 
        'target_availabilities', 'history_positions', 'history_yaws', 'history_availabilities', 'curr_speed', 'centroid', 'yaw', 'type', 'extent', 
        ...
        'all_other_agents_history_yaws', 'all_other_agents_history_availability', 'all_other_agents_history_availabilities', 'all_other_agents_curr_speed', 
        'all_other_agents_future_positions', 'all_other_agents_future_yaws', 'all_other_agents_future_availability', 'all_other_agents_types', 
        'all_other_agents_extents', 'all_other_agents_history_extents', 'scene_index', 'track_id', 'ego_idx', 'ego_plan'
        """
...
class RolloutWrapper(Wrappers):
    ...
    def get_action(self, obs, step_index) -> RolloutAction:
        """
        根据观测数据和时间步索引，计算 ego 和其他智能体的动作。

        参数:
            obs: dict,包含 ego 和其他智能体的观测数据。
            step_index: int,当前时间步索引。

        返回:
            RolloutAction: 包含 ego 和其他智能体动作及其信息的对象。
        """
        ego_action = None
        ego_action_info = None
        agents_action = None
        agents_action_info = None
        # print("ego policy:", self.ego_policy, type(self.ego_policy))
        # print("agents policy:", self.agents_policy, type(self.agents_policy))
        # print("obs:", obs.keys(), type(obs))

        # 默认全部执行
        # 预测 ego 的动作
        if self.ego_policy is not None: # ego_policy: Pos2YawWrapper
        ...
        # 预测 agents 的动作
        if self.agents_policy is not None: # Pos2YawWrapper
