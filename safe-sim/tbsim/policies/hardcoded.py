...
class StrivePlanner_trajdata(Policy):
    def __init__(self, device,dt=0.1,*args, **kwargs):
        ...
        # 加载 nuScenes 数据集的车道图（包含车道中心线、拓扑关系等）
        # load_lane_graphs_trajdata 返回一个字典，键为地图名称，值为 LaneGraph 对象
    ...
    def get_action(self, obs_dict, **kwargs):
        """
        根据当前观测生成控制动作。不依赖神经网络,利用预加载的车道图和一个确定性规划器HardcodeNuscPlanner规划安全的行驶路径。

        Args:
            obs_dict: 观测字典，包含以下关键字段：
                - agent_hist: 自车历史状态，形状 [batch_size, hist_len, 特征数]。
                - map_names: 每个场景的地图名称列表，用于选择对应的车道图。
                - num_neigh: 每个场景中实际邻居数量。
                - neigh_hist: 邻居历史状态（填充至最大邻居数），形状 [batch_size, max_neighbors, hist_len, 特征数]。
                - world_from_agent: 从智能体局部坐标系到世界坐标系的变换矩阵（每个场景不同），形状 [batch_size, 3, 3]。
                - agents_from_world_tf: 从世界坐标系到智能体局部坐标系的变换矩阵（每个场景不同），形状 [batch_size, 3, 3]。
                - 其他可选字段（如 agent_fut 等，未使用）。
            **kwargs: 额外参数。

        Returns:
            action: 一个 Action 对象，包含 positions (形状 [batch_size, output_len, 2]) 和 yaws (形状 [batch_size, output_len, 1])。
            info: 空字典。
        """
        ...
        for i in range(batch_size):
            ...
            hist_len = self.cfg["hist_len"]             # 历史帧数
            fut_len = self.cfg["fut_len"]               # 未来规划帧数
            num_neighbors = obs_dict["num_neigh"][i]    # 实际邻居数量

            # 邻居历史状态：形状 [max_neighbors, hist_len, 特征数]
            ...
            # 安全校验：检查当前时刻（索引 hist_len）的邻居状态是否全零，
            # 若全零则意味着这些邻居与自车位置重叠，会导致规划失效。
